...
Ao testar pelo painel uma integração configurada com `API_KEY`, a chave deve ser informada no modal de teste. A requisição utiliza o mesmo header `X-API-Key` exigido de consumidores externos.

**### Exemplos de consumo**

Integração sem autenticação (`NONE`):

```http
GET /api/pedidos/itensPedido?pedido_id=1
```

Em ambiente local, também pode ser acessada diretamente pelo navegador:

```text
http://localhost:8081/api/pedidos/itensPedido?pedido_id=1
```

Integração protegida por API Key:

```http
GET /api/pedidos2/itensPedido?pedido_id=1
X-API-Key: ihub_sua_api_key
```

No Postman, informe a API Key diretamente em **Headers**:

```text
Key:   X-API-Key
Value: ihub_sua_api_key
```

Exemplo utilizando JavaScript no navegador:

```javascript
fetch('http://localhost:8081/api/pedidos2/itensPedido?pedido_id=1', {
  headers: {
    'X-API-Key': 'ihub_sua_api_key'
  }
})
...
A API Key não deve ser enviada como parâmetro da URL. O consumo deve utilizar o header HTTP `X-API-Key`.
...
As tabelas próprias da aplicação utilizam o prefixo \`IH\_\`.
